...
@Component({
  ...
  providers: [CustomFieldsService, MatomoTracker]
})
export class CustomdataComponent implements OnInit {
  ...
  ngOnInit() {
    this.matomoTracker.trackPageView("Community Manager: My Trading Community - Custom Data");
  ...
  customizedGroupdata() {
    ...
    this.FieldGroups.forEach(group => {
      group.url = "customdata/partner/withAnswer?groupId=" + group.uniqueId + "&partnerBuId=" + this.companyId;
    });
